...
// allBuildFiles returns the names of all files used for building the package
// in the current configuration. There may be duplicates in the list
// if source files are also used for //go:embed.
func (p *Package) allBuildFiles() []string {
...
func resolveEmbed(fsys fs.FS, patterns []string) (files []string, pmap map[string][]string, err error) {
	...
	for _, pattern = range patterns {
		...
		// TODO(rsc): How can Glob return the same file twice?
		// Or else why do we need to check have[file] != pid?
	...
	files = slices.Sorted(maps.Keys(have))
...
func isBadEmbedName(name string) bool {
	...
	// Keep .bzr for now since we previously disallowed it
...
func (p *Package) setBuildInfo(ctx context.Context, f *modfetch.Fetcher, autoVCS bool) {
	...
	pdeps := PackageList([]*Package{p})
	mdeps := make(map[module.Version]*debug.Module)
	var q []*Package
	q = append(q, p.Internal.Imports...)
	for _, p1 := range pdeps {
		if p1.Module != nil {
	...
	if repoDir != "" && vcsCmd.Status != nil {
		...
		dirty, err := dirtyBuild(pdeps, vcsCmd, repoDir, st)
		if err != nil {
			setVCSError(err)
			return
		}
		appendSetting("vcs.modified", fmt.Sprint(dirty))
...
// dirtyBuild reports whether a build should be considered dirty
// due to uncommitted modifications to files in the VCS repository dir.
// pdeps is the list of all packages used in the build.
func dirtyBuild(pdeps []*Package, vcsCmd *vcs.Cmd, repoDir string, st vcs.Status) (bool, error) {
	if len(st.Added)+len(st.Modified)+len(st.Removed) == 0 {
		return false, nil
	}

	// Index modified files for O(1) lookup.
	modified := make(map[string]bool)
	for _, file := range st.Modified {
		modified[filepath.ToSlash(file)] = true
	}
	for _, file := range st.Added {
		modified[filepath.ToSlash(file)] = true
	}

	// Phase 1. Check for modified files in the current build.
	// This part can be done without VCS operations.
	for _, p := range pdeps {
		dir, err := filepath.Rel(repoDir, p.Dir)
		if err != nil || strings.HasPrefix(dir, ".."+string(filepath.Separator)) {
			// p is outside repoDir.
			continue
		}
		dir = filepath.ToSlash(dir)

		// Look for modified files used in build.
		for _, file := range p.allBuildFiles() {
			if modified[path.Join(dir, file)] {
				return true, nil
			}
		}

		// Look for modified files matching embed patterns.
		// (The only possibility here is removed files,
		// or else they'd be in allBuildFiles too.)
		// This is somewhat unavoidably O(#Patterns * #Removed).
		// We could filter somewhat by directory if it became necessary
		// but each file has to be matched against all its parent directories' embeds.
		for _, pattern := range p.EmbedPatterns {
			if dir != "." {
				pattern = dir + "/" + pattern
			}
			for _, file := range st.Removed {
	...
	// Phase 2. Check for modified files in the unmodified build.
	// (If we get this far, there are no modified files in the current build,
	// so the only possibility is files that have been removed or otherwise
	// edited to stop being part of the build.)
	// For this we need to inspect the original contents using the VCS.
	var vcsErr error
	ctxt := build.Context(cfg.BuildContext) // no-op conversion but be clear: struct copy, not a pointer
	ctxt.OpenFile = func(file string) (io.ReadCloser, error) {
		rel, err := filepath.Rel(repoDir, file)
		if err != nil {
			return nil, fmt.Errorf("file not found") // impossible, we know it is in repoDir
		}
		data, err := vcsCmd.ReadFile(repoDir, rel)
		if err != nil {
			if vcsErr == nil {
				vcsErr = err
			}
	...
	for _, file := range st.Modified {
		dir := filepath.Join(repoDir, filepath.Dir(file))
	...
	for _, p := range pdeps {
		for _, file := range byDir[p.Dir] {
			// Note: Ignoring error here and relying on saving error in
			// our file wrapper instead, because we don't care about
			// errors like parse failures inside the original file.
			// We only care about failure to read from VCS.
			name := filepath.Base(file)
			matched, _ := ctxt.MatchFile(p.Dir, name)
			if matched {
				return true, nil
			}
		}
